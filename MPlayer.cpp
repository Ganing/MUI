...
struct AudioVisConfig {
    int fftWindowSize = 512;     // FFT 窗口大小
    int fftBins = 256;           // 频段数 = windowSize / 2
    int barCount = 64;           // 条数
    float updateIntervalMs = 33.0f; // FFT 分析间隔 (ms)
    float decayRate = 0.92f;
    float overlayAlpha = 0.70f;
    float barWidth = 0.55f;
    float barGap = 0.45f;
    float bottomMargin = 0.04f;
    float barAreaHeight = 0.30f;
    float minBarHeight = 0.017f;
    float barColor[3] = { 0.0f, 0.82f, 0.41f };
    float peakColor[3] = { 1.0f, 0.55f, 0.0f };
};

// 环形缓冲：单写（音频回调）单读（主线程）
struct AudioVisRingBuffer {
    std::vector<float> buf;
    std::atomic<size_t> writePos{0};
    size_t cap;
    explicit AudioVisRingBuffer(size_t n) : buf(n, 0.0f), cap(n) {}
    AudioVisRingBuffer() : cap(0) {}
    ...
    size_t read(std::vector<float>& dst, size_t n) {
        size_t wp = writePos.load(std::memory_order_acquire);
        size_t start = (wp >= n) ? (wp - n) : (cap - (n - wp));
        dst.resize(n);
        for (size_t i = 0; i < n; i++) {
            size_t idx = start + i;
            if (idx >= cap) idx -= cap;
...
struct AppState {
    ...
    int pendingLoadFileIdx = -1;  // -1 = none, deferred from dropdown callback
    ...
    // --- 辅助 GL 上下文（独立 HGLRC，不受 wallpaper 切换影响） ---
    ...
    // --- Lyrics FBO + PBO（辅助 HGLRC 中，异步回读） ---
    GLuint lrcFboId = 0;
    GLuint lrcTexId = 0;
    tvg::GlCanvas* lrcGlCanvas = nullptr;
    tvg::Scene* lrcTvgScene = nullptr;
    int lrcFboW = 0, lrcFboH = 0;
    // PBO 双缓冲
    GLuint lrcPboIds[2] = {0, 0};
    int lrcPboIndex = 0;
    int lrcPboReadyIndex = -1;
    std::vector<uint8_t> lrcPboPixelBuf;
    Texture* lrcFilamentTex = nullptr;
    bool lrcFboDirty = true;
    int lrcLastTexW = 0, lrcLastTexH = 0;  // track Filament tex dimensions for resize

    // --- OUI FBO + PBO（辅助 HGLRC 中，渲染 UI + AudioBars） ---
    GLuint ouiFboId = 0;
    GLuint ouiTexId = 0;
    int ouiFboW = 0, ouiFboH = 0;
    GLuint ouiPboIds[2] = {0, 0};
    int ouiPboIndex = 0;
    int ouiPboReadyIndex = -1;
    std::vector<uint8_t> ouiPboPixelBuf;
    Texture* ouiFilamentTex = nullptr;
    Entity ouiEntity;
    Material* ouiMat = nullptr;
    MaterialInstance* ouiMi = nullptr;
    bool ouiFboDirty = true;
    int ouiLastTexW = 0, ouiLastTexH = 0;

    // --- 频谱条（FFT 分析保留，渲染改为 ThorVG → OUI FBO） ---
...
/*==================== 歌词 FBO 渲染（辅助 HGLRC + PBO 双缓冲异步回读） ====================*/

static void destroyLrcFbo(AppState& st) {
    if (!wglMakeCurrent(st.auxDC, st.auxGLRC)) return;
    if (st.lrcGlCanvas) {
        st.lrcGlCanvas->remove(nullptr);
        delete st.lrcGlCanvas;
        st.lrcGlCanvas = nullptr;
    }
    if (st.lrcTvgScene) { delete st.lrcTvgScene; st.lrcTvgScene = nullptr; }
    if (st.lrcPboIds[0]) { glDeleteBuffers(2, st.lrcPboIds); st.lrcPboIds[0] = st.lrcPboIds[1] = 0; }
    ...
    if (st.lrcFilamentTex && st.engine) { st.engine->destroy(st.lrcFilamentTex); st.lrcFilamentTex = nullptr; }
    st.lrcFboW = st.lrcFboH = 0;
    st.lrcPboReadyIndex = -1;
    st.lrcPboIndex = 0;
...
static bool createLrcFbo(AppState& st, int w, int h) {
    ...
    if (!wglMakeCurrent(st.auxDC, st.auxGLRC)) return false;

    // 销毁旧资源
    if (st.lrcFboId != 0) {
        if (st.lrcGlCanvas) { st.lrcGlCanvas->remove(nullptr); delete st.lrcGlCanvas; st.lrcGlCanvas = nullptr; }
        if (st.lrcTvgScene) { delete st.lrcTvgScene; st.lrcTvgScene = nullptr; }
        if (st.lrcPboIds[0]) { glDeleteBuffers(2, st.lrcPboIds); st.lrcPboIds[0] = st.lrcPboIds[1] = 0; }
        if (st.lrcTexId) { glDeleteTextures(1, &st.lrcTexId); st.lrcTexId = 0; }
        if (st.lrcFboId) { glDeleteFramebuffers(1, &st.lrcFboId); st.lrcFboId = 0; }
    ...
    glGenBuffers(2, st.lrcPboIds);
    for (int i = 0; i < 2; ++i) {
        glBindBuffer(GL_PIXEL_PACK_BUFFER, st.lrcPboIds[i]);
        glBufferData(GL_PIXEL_PACK_BUFFER, bufSize, nullptr, GL_STREAM_READ);
    }
    glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
    st.lrcPboPixelBuf.resize(bufSize);
    st.lrcPboIndex = 0;
    st.lrcPboReadyIndex = -1;
    ...
    if (!st.lrcGlCanvas) { wglMakeCurrent(nullptr, nullptr); return false; }
    auto r = st.lrcGlCanvas->target(st.auxGLRC, st.lrcFboId, (uint32_t)w, (uint32_t)h, tvg::ColorSpace::ABGR8888S);
    if (r != tvg::Result::Success) { wglMakeCurrent(nullptr, nullptr); return false; }

    st.lrcTvgScene = tvg::Scene::gen();
    if (!st.lrcTvgScene) { wglMakeCurrent(nullptr, nullptr); return false; }

    st.lrcFboW = w;
    st.lrcFboH = h;
    st.lrcFboDirty = true;
    wglMakeCurrent(nullptr, nullptr);
    OX_LOG("[LRC] FBO+PBO created in aux context: %dx%d\n", w, h);
    return true;
}
...
// PBO 异步回读 + 上传到 Filament 纹理（通用，用于 Lyrics 和 OUI）
// 必须在 auxDC/auxGLRC 为当前上下文时调用
static void uploadFboWithPbo(Engine* engine, GLuint fboId, Texture*& tex, int texW, int texH,
                              int& lastTexW, int& lastTexH,
                              TextureSampler smp, MaterialInstance* targetMi, const char* paramName,
                              GLuint* pboIds, int& pboIdx, int& readyIdx,
                              std::vector<uint8_t>& pixelBuf, bool& dirtyFlag) {
    if (!fboId || !engine) return;

    // 重建 Filament 纹理（尺寸变化时）
    if (!tex || texW != lastTexW || texH != lastTexH) {
        if (tex) { engine->destroy(tex); tex = nullptr; }
        tex = Texture::Builder()
            .width((uint32_t)texW).height((uint32_t)texH).levels(1)
        ...
        lastTexH = texH;
        if (tex && targetMi) {
            targetMi->setParameter(paramName, tex, smp);
        }
    }
    if (!tex || !pboIds[0]) return;

    // 读取上一帧就绪的 PBO
    if (readyIdx >= 0) {
        glBindBuffer(GL_PIXEL_PACK_BUFFER, pboIds[readyIdx]);
        void* mapped = glMapBuffer(GL_PIXEL_PACK_BUFFER, GL_READ_ONLY);
        if (mapped) {
            size_t bufSize = pixelBuf.size();
            memcpy(pixelBuf.data(), mapped, bufSize);
            glUnmapBuffer(GL_PIXEL_PACK_BUFFER);

            uint8_t* pixelData = new uint8_t[bufSize];
            memcpy(pixelData, pixelBuf.data(), bufSize);
            tex->setImage(*engine, 0,
                Texture::PixelBufferDescriptor(pixelData, bufSize,
                    Texture::PixelBufferDescriptor::PixelDataFormat::RGBA,
                    Texture::PixelBufferDescriptor::PixelDataType::UBYTE,
                    [](void* buf, size_t, void*) { delete[] (uint8_t*)buf; }, nullptr));
        }
        glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
    }

    // 将当前 FBO 内容异步读入当前 PBO
    glBindFramebuffer(GL_READ_FRAMEBUFFER, fboId);
    glBindBuffer(GL_PIXEL_PACK_BUFFER, pboIds[pboIdx]);
    glReadPixels(0, 0, texW, texH, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
...
/*==================== 辅助 OpenGL 上下文 ====================*/
// 创建独立的隐藏辅助窗口 + HGLRC（参考 momo v1.1 模式）
// 独立的 HGLRC 不受 wallpaper 切换影响，不共享 display list（PBO 中转）
static bool createAuxGLContext(AppState& st) {
    WNDCLASSEXW wc = {};
    wc.cbSize = sizeof(WNDCLASSEXW);
    wc.lpfnWndProc = DefWindowProcW;
    wc.hInstance = GetModuleHandleW(nullptr);
    wc.lpszClassName = L"MPAuxHelperWindow";
    RegisterClassExW(&wc);

    st.auxHwnd = CreateWindowExW(0, L"MPAuxHelperWindow", L"", WS_OVERLAPPEDWINDOW,
...
/*==================== OUI FBO（辅助 HGLRC 中，用于 UI + AudioBars 离屏渲染） ====================*/

static void destroyOuiFbo(AppState& st) {
    if (!wglMakeCurrent(st.auxDC, st.auxGLRC)) return;
    if (st.ouiPboIds[0]) { glDeleteBuffers(2, st.ouiPboIds); st.ouiPboIds[0] = st.ouiPboIds[1] = 0; }
    if (st.ouiTexId) { glDeleteTextures(1, &st.ouiTexId); st.ouiTexId = 0; }
    if (st.ouiFboId) { glDeleteFramebuffers(1, &st.ouiFboId); st.ouiFboId = 0; }
    if (st.ouiFilamentTex && st.engine) { st.engine->destroy(st.ouiFilamentTex); st.ouiFilamentTex = nullptr; }
    st.ouiFboW = st.ouiFboH = 0;
    st.ouiLastTexW = st.ouiLastTexH = 0;
    st.ouiPboReadyIndex = -1;
    st.ouiPboIndex = 0;
    wglMakeCurrent(nullptr, nullptr);
}
...
static bool createOuiFbo(AppState& st, int w, int h) {
    ...
    glGenTextures(1, &st.ouiTexId);
    glBindTexture(GL_TEXTURE_2D, st.ouiTexId);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, st.ouiTexId, 0);
    ...
    size_t bufSize = (size_t)w * h * 4;
    glGenBuffers(2, st.ouiPboIds);
    for (int i = 0; i < 2; ++i) {
        glBindBuffer(GL_PIXEL_PACK_BUFFER, st.ouiPboIds[i]);
        glBufferData(GL_PIXEL_PACK_BUFFER, bufSize, nullptr, GL_STREAM_READ);
    }
    glBindBuffer(GL_PIXEL_PACK_BUFFER, 0);
    st.ouiPboPixelBuf.resize(bufSize);
    st.ouiPboIndex = 0;
    st.ouiPboReadyIndex = -1;
    ...
    st.ouiFboDirty = true;
    wglMakeCurrent(nullptr, nullptr);
    OX_LOG("[OUI] OUI FBO+PBO created in aux context: %dx%d\n", w, h);
    return true;
}

/*==================== AudioBars ThorVG 渲染（在 OUI FBO 中绘制频谱条） ====================*/
static void renderAudioBarsToScene(tvg::Scene* scene, AppState& st, float winW, float winH) {
    ...
    // 平滑 bar heights（attack/release）
    float dtClamped = 0.016f;  // ~60fps
    ...
    for (int ib = 0; ib < cfg.barCount; ib++) {
        ...
        float barY = barAreaTop + (barAreaH - barH);  // bars grow upward from bottom
...
static void updateVisibleLayers(AppState& st) {
    ...
    // OUI overlay (includes UI widgets + AudioBars when enabled)
    mask |= 0x10;
    st.view->setVisibleLayers(0xFF, mask);
}

/*==================== OUI 叠加层 Material（简单 alpha blend） ====================*/
static Material* buildOuiOverlayMaterial(Engine* engine) {
    filamat::MaterialBuilder b;
    b.name("OuiOverlay")
     .material(R"FILAMENT(
void material(inout MaterialInputs m) {
    ...
    vec2 uv = getUV0();
    uv.y = 1.0 - uv.y;  // ThorVG FBO → glReadPixels flips Y
    m.baseColor = texture(materialParams_ouiTex, uv);
}
)FILAMENT")
     .parameter("ouiTex", filamat::MaterialBuilder::SamplerType::SAMPLER_2D)
     .require(filament::VertexAttribute::UV0)
     .shading(Shading::UNLIT)
     .blending(filamat::MaterialBuilder::BlendingMode::TRANSPARENT)
     .depthCulling(false)
     ...
     .platform(filamat::MaterialBuilder::Platform::DESKTOP);
    filamat::Package pkg = b.build(engine->getJobSystem());
    if (!pkg.isValid()) { OX_LOG("[OUI] OuiOverlay material compile FAILED\n"); return nullptr; }
    OX_LOG("[OUI] OuiOverlay material compiled OK\n");
    return Material::Builder().package(pkg.getData(), pkg.getSize()).build(*engine);
}

/*==================== FFT 频谱分析（对数频率聚合，tem.txt 同款）====================*/
static void analyzeSpectrum(AppState& st, double audioTime) {
    ...
    for (int b = 0; b < barCount; b++) {
        ...
        float fCenter = powf(10.0f, logMin + frac * logRange);
...
static void cleanup(AppState& st) {
    ...
    if (st.engine) {
        ...
        if (st.ouiEntity) st.engine->destroy(st.ouiEntity);
        if (st.ouiMi) st.engine->destroy(st.ouiMi);
        if (st.ouiMat) st.engine->destroy(st.ouiMat);
        if (st.ouiFilamentTex) st.engine->destroy(st.ouiFilamentTex);
        fft_free(st.avFft);
...
static bool loadMediaFile(AppState& st, const char* path, int winW, int winH) {
    ...
    // --- Lyrics FBO texture is now created by uploadFboWithPbo in render loop ---
    // (was: Bind lyrics FBO texture to HeartfeltRain FX material - now unified)
...
int main() {
    ...
    // ---------- 1.2. Auxiliary GL context ----------
    if (!createAuxGLContext(st)) {
        OX_LOG("[MAIN] Aux GL context create failed\n");
    ...
    // ---------- Build AudioBars overlay (on top of everything, layer 0x10) ----------
    {
    }
    ...
    // FFT init for spectrum analysis (bars rendered via ThorVG into OUI FBO)
    {
        auto& cfg = st.avConfig;
        ...
        OX_LOG("[AVBars] FFT init: fftBins=%d barCount=%d ringCap=%zu\n", cfg.fftBins, cfg.barCount, ringCap);
    }

    // Create OUI overlay material + entity
    {
        st.ouiMat = buildOuiOverlayMaterial(st.engine);
        if (st.ouiMat) {
            st.ouiMi = st.ouiMat->createInstance();
            st.ouiEntity = createFullscreenQuad(st.engine, st.scene, st.ouiMi);
            if (st.ouiEntity) {
                auto& rcm = st.engine->getRenderableManager();
                auto oi = rcm.getInstance(st.ouiEntity);
                rcm.setLayerMask(oi, 0xFF, 0x10);
                rcm.setPriority(oi, 5);
            }
        }
    }
    ...
    // Create OUI FBO first, then init OUI with it
    createOuiFbo(st, (int)width, (int)height);
    // ThorVG GlCanvas::gen() needs a current GL context
    wglMakeCurrent(st.auxDC, st.auxGLRC);
    if (!st.ui.initFbo(st.auxGLRC, st.ouiFboId, (int)width, (int)height)) {
        wglMakeCurrent(nullptr, nullptr);
        OX_LOG("[MAIN] OUI initFbo failed\n");
        tvg::Initializer::term(); cleanup(st); destroyAuxGLContext(st); st.app.destroy(); timeEndPeriod(1); return 1;
    }
    wglMakeCurrent(nullptr, nullptr);
    ...
    rebuildUI = [&]() {
        ...
        addL("MPlayer--作者：元亨利贞", 28.0f, OX::OColor(100, 200, 255, 255));
        ...
        if (!st.mediaFiles.empty()) {
            ...
            dd->onSelectionChanged = [&st](int idx, const std::string&) {
                ...
                // 延迟到主循环处理，避免回调中直接 loadMediaFile 导致重入崩溃
        ...
        // 不在 rebuildUI 中执行 ThorVG 渲染，避免与 Filament 状态变更竞态。
        // 仅设置 dirty 标记，主循环渲染段会处理实际 FBO 渲染。
        if (st.showLyrics) {
    ...
    st.app.setSizeCallback([&](uint32_t w, uint32_t h) {
        {
            ...
            // Resize FBOs in auxiliary context
    ...
    while (!st.app.shouldClose()) {
        ...
        // Recreate swap chain after wallpaper mode toggle (window style changed)
        // OBS/recording software may hook into the graphics pipeline, causing
        // createSwapChain/destroy to fail. Handle gracefully.
        if (st.needSwapChainRecreate) {
            st.needSwapChainRecreate = false;
            if (st.engine && st.swapChain) {
                st.engine->flushAndWait();
                // Disable rendering until swap chain is recreated
                auto* oldSc = st.swapChain;
                st.swapChain = nullptr;
                st.engine->destroy(oldSc);
                auto* newSc = st.engine->createSwapChain((void*)st.app.getHwnd());
                if (newSc) {
                    st.swapChain = newSc;
                } else {
                    // Retry next frame if creation failed (OBS hook may have interfered)
                    st.needSwapChainRecreate = true;
                }
            }
        }
        ...
        // === Audio bars: FFT analysis (CPU) - run BEFORE ThorVG rendering ===
        if (st.showAudioBars) {
            double audioTime = (double)st.audioSamplesPlayed / (double)st.AUDIO_SAMPLE_RATE;
            if (audioTime - st.avLastFftTime >= (double)st.avConfig.updateIntervalMs * 0.001) {
                analyzeSpectrum(st, audioTime);
                st.avLastFftTime = audioTime;
            }
        }

        // === ThorVG rendering in auxiliary GL context (before Filament) ===
        // 处理延迟的文件加载（来自下拉框选择，避免回调重入崩溃）
        ...
        if (st.needRebuildUI) {
            st.needRebuildUI = false; rebuildUI();
        }
        st.ui.update(dt);
        if (wglMakeCurrent(st.auxDC, st.auxGLRC)) {
            // --- Lyrics FBO ---
            // 注意：PBO 有一帧延迟（glReadPixels→PBO→下帧映射上传），
            // 因此 dirty 渲染和 PBO 上传必须分开：
            //   - dirty 时渲染 FBO + 启动 glReadPixels→PBO
            //   - 非 dirty 时也要调用 upload 从就绪的 PBO 上传到 Filament 纹理
            // 否则首帧渲染的歌词数据永远到不了 Filament 纹理。
            if (st.lrcFboDirty) {
                ...
                auto smpLrc = TextureSampler(TextureSampler::MinFilter::LINEAR, TextureSampler::MagFilter::LINEAR);
                uploadFboWithPbo(st.engine, st.lrcFboId, st.lrcFilamentTex, st.lrcFboW, st.lrcFboH,
                    st.lrcLastTexW, st.lrcLastTexH, smpLrc, st.lyricsMi, "lyricsTex",
                    st.lrcPboIds, st.lrcPboIndex, st.lrcPboReadyIndex, st.lrcPboPixelBuf, st.lrcFboDirty);
            } else if (st.lrcFboId && st.lrcPboIds[0]) {
                // 无需重渲染 FBO，仅从已就绪的 PBO 上传到 Filament 纹理
                auto smpLrc = TextureSampler(TextureSampler::MinFilter::LINEAR, TextureSampler::MagFilter::LINEAR);
                uploadFboWithPbo(st.engine, st.lrcFboId, st.lrcFilamentTex, st.lrcFboW, st.lrcFboH,
                    st.lrcLastTexW, st.lrcLastTexH, smpLrc, st.lyricsMi, "lyricsTex",
                    st.lrcPboIds, st.lrcPboIndex, st.lrcPboReadyIndex, st.lrcPboPixelBuf, st.lrcFboDirty);
            ...
            // Bind lyrics texture to FX materials (if they were created)
            if (st.lrcFilamentTex && st.fxEffectCount > 0) {
                for (int i = 0; i < st.fxEffectCount; i++) {
                    if (st.fxMi[i]) {
                        st.fxMi[i]->setParameter("lyricsTex", st.lrcFilamentTex,
                            TextureSampler(TextureSampler::MinFilter::LINEAR, TextureSampler::MagFilter::LINEAR));
                    }
                }
            }

            // --- OUI FBO (UI widgets + AudioBars) ---
            {
                // UI 渲染完成后，通过回调将音频条渲染到同一 rootScene（在最上层）
                ...
                auto smpOui = TextureSampler(TextureSampler::MinFilter::LINEAR, TextureSampler::MagFilter::LINEAR);
                uploadFboWithPbo(st.engine, st.ouiFboId, st.ouiFilamentTex, st.ouiFboW, st.ouiFboH,
                    st.ouiLastTexW, st.ouiLastTexH, smpOui, st.ouiMi, "ouiTex",
                    st.ouiPboIds, st.ouiPboIndex, st.ouiPboReadyIndex, st.ouiPboPixelBuf, st.ouiFboDirty);
            }
        }
        wglMakeCurrent(nullptr, nullptr);

        // === Filament render ===
        if (st.swapChain && st.renderer->beginFrame(st.swapChain)) {
            st.renderer->render(st.view);
            st.renderer->endFrame();
        }
        st.engine->flushAndWait();

        // === Swap buffers (OUI now rendered as Filament quad, no more direct GL rendering) ===
        st.app.swapBuffers();
    }
