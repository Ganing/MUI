...
public:
    /**
     * @brief Structure containing metrics for a single glyph
     * aeglove 2025-10-22
     * @since 1.1
     */
    struct TextGlyphInfo {
        uint32_t codepoint;     ///< Unicode codepoint
        float x;                ///< X position in layout (scaled)
        float y;                ///< Y position in layout (scaled)
        float advance;          ///< Horizontal advance width (scaled)
        float width;            ///< Glyph bounding box width (scaled)
        float height;           ///< Glyph bounding box height (scaled)
    };

    /**
     * @brief Get metrics for each glyph in the text
     * aeglove 2025-10-22
     * @param[out] glyphInfo Vector to receive glyph information
     * @return Result::Success on success
     * @return Result::InsufficientCondition if no font or text is set
     *
     * @note Experimental API
     * @since 1.1
     */
    Result getGlyphInfo(std::vector<TextGlyphInfo>& glyphInfo) noexcept;
public:
    /**
